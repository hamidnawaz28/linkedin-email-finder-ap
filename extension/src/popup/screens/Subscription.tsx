...
const Subscription = ({ children, setUser }: any) => {
    ...
    const verifyHandle = async () => {
        ...
        if (!tokenDetails) {
            ...
            updateLocalData({ user: tokenDetails })
    ...
    return (
        <Box>
            <Box sx={{
                ...
                <Button variant='contained' size='small' color='error' onClick={() => otherBrowerMethods.createATab('https://www.google.com')}>Subscribe</Button>
            </Box>
            <div>{children}</div>
        </Box >
    )
